...
void DatObject::dump(){
	...
	cout<<"frames = "<<(uint32_t)frames<<endl;
...
bool DatLoader::getObject(DatObject& obj, uint32_t id){
	...
	obj.frames = file->getUint8();
	obj.divX = file->getUint8();
	obj.divY = file->getUint8();
	obj.divZ = file->getUint8();
	obj.animatedFrames = file->getUint8();
	uint64_t sprites = obj.width * obj.height * obj.frames * obj.divX * obj.divY * obj.animatedFrames * obj.divZ;
